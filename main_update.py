...
# This comes from main.py (feature-rich)
# ---------------------------
class MazeApp:
    def __init__(self, root):
        ...
        self.cream = "#EEC276"       # dark cream background

        # UI colors
        self.neon_bg = self.cream
        self.panel_bg = self.cream
        self.glow_cyan = self.teal
        self.glow_purple = self.dark_blue
        self.button_cyan = self.teal
        self.button_purple = self.yellow
        self.text_light = self.dark_blue
        self.grid_bg = self.cream
    ...
    def draw_grid(self):
        ...
        for i in range(self.n):
            for j in range(self.n):
                ...
                fill = self.cream  # Dark cream background for empty cells
                if (i, j) == self.start:
                    fill = self.teal  # Deep teal for start
                elif (i, j) == self.goal:
                    fill = self.red  # Deep red for goal
                elif (i, j) in self.walls:
                    fill = self.dark_blue  # Very dark blue for walls
    ...
    def run(self, algo_name, mode, func):
        ...
        def task():
            ...
            def show_result_window(title, message, success=True):
                ...
                win.configure(bg=self.glow_cyan)
                win.resizable(False, False)

                icon_frame = tk.Frame(win, bg=self.glow_purple)
    ...
    def show_results_table(self):
        ...
        tree.tag_configure("oddrow", background=self.panel_bg, foreground=self.text_light)
    ...
    # ---------------- Plotly/Matplotlib chart in tkinter window ----------------
    def show_chart_in_window(self):
        """Display the chart in a new tkinter window (matplotlib embedded)"""
        if not self.results:
        ...
        # Colors for the bars (use hex strings; matplotlib will accept them)
...
# ---------------------------
# StartScreen (Fixed + Background resize)
# Uses the improved StartScreen from main2 but with layering fixes
# ---------------------------
class StartScreen:
    def __init__(self):
        self.root = tk.Tk()
        ...
        # Set your background image path here if desired
        import os
        ...
        # Title layers (shadow, outline, neon)
        # Shadow (slightly lower)
        ...
        # Start button with border frame for aesthetic
        ...
        make_neon_button(self.start_btn, "#f8edce", "#ff6b6b")
    ...
    def launch_main(self):
        # Destroy splash and open main app (MazeApp)
        try:
            self.root.destroy()
        except:
            pass
